...
def verify_database():
   ...
   for user in c.execute('SELECT * FROM  Utilisateurs'):
      if not(user_is_valid(user["Username"],user["Password"])):
         print("Invalid user: ",user["Username"])
         return False

   #Vérifier les liens
   for link in c.execute('SELECT * FROM  Liens'):
      for char in link["Link"]:
         if not(char in smallalphabets or char in capitalalphabets or char in digits):
            print("Invalid link: ",link["Link"])
            return False
